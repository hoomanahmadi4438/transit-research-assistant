 ...
 * dense/sparse rerankers. For each test question we compare:
 *
 *   BASELINE — retrieve() run once on the question as originally phrased.
 *   POOLED   — retrieve() run on the original question AND two differently
 *              worded reformulations, then the results are pooled (by
 *              taking each chunk's best score across the three runs) and
 *              truncated to the SAME top-k budget as the baseline.
 *
